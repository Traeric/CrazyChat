		...
		methods: {
			...
			onSubmit() {
				...
				if (Type === '0') {
					...
					let to = this.toUser;
					if (from === "" || to === "") {
						this.$message.info('必须选择用户！');
						return;
					}
					if (from.trim() === to.trim()) {
						this.$message.error('不能查看用户跟用户自己的聊天记录！');
						return;
					}
					// 获取时间
					let date = this.date;
					let startTime = Date.parse(date[0]) || 0;
					let endTime = Date.parse(date[1]) || 0;
					chatApi.user2user(from, to, startTime, endTime).then((response) => {
						this.$message({
					...
					chatApi.user2groupAll(user, groupId, startTime, endTime).then((response) => {
						this.$message({
							message: response.data.message,
							type: (response.data.flag ? 'success' : 'error'),
						});
